...
class MaskedConv1d(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, kernel_size: int=-1, mask: List[int]=[]) -> None:
        ...
        layer = nn.Conv1d(in_dim, out_dim, kernel_size=kernel_size, padding=self._padding)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return nn.functional.conv1d(x, kernel, bias=self._bias, padding=self._padding)
